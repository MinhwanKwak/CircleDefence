...
namespace oddments
{
    ...
    public class TableCreatorWindow : EditorWindow
	{
		...
		private const string ScriptSavePath = "oddments/Scripts/Table/Define";
		private const string TableResourceSavePath = "Assets/oddments/TableAsset";
		...
		{
			var scriptSample = (TextAsset)AssetDatabase.LoadAssetAtPath("Assets/oddments/Editor/TableSample.txt", typeof(TextAsset));
...
";
		...
		void CreateTableAsset() {
			...
			var type = Type.GetType($"oddments.{table_name},oddments");
			var dataType = Type.GetType($"oddments.{table_name}Data,oddments");
